...
- SmartFlux-like runtime hardening now has a headless runtime profile/watchdog manifest plus runtime_service v1 for queued headless batches, heartbeats, disk/queue telemetry, retry records, quarantined failures, delivery state, process telemetry, supervisor status, PTP/GPS logs, hardware watchdog event logs, supervisor_integration v1 for systemd/Windows/manual status plus file/manual hardware watchdog kick/reboot handoff, and installable_runtime_profile v1 for auditable systemd/Windows Service dry-run deployment plans across report center, formal reports, delivery package, and exporter manifests; a privileged installed embedded daemon executor and direct hardware watchdog/reboot providers remain incomplete.
