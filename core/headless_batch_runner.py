...
def run_cli(argv: list[str] | None = None) -> int:
    ...
    if args.build_public_ec_acquisition_runbook:
        return _run_public_ec_acquisition_runbook_cli(args, parser)
...
def _run_eddypro_computation_scope_audit_cli(args: argparse.Namespace, parser: argparse.ArgumentParser) -> int:
    if not args.output:
        parser.error("--output is required with --build-eddypro-computation-scope-audit.")
    workspace_root = Path(args.workspace_root) if args.workspace_root else None
    coverage = (
        load_config_file(args.eddypro_coverage_audit)
        if getattr(args, "eddypro_coverage_audit", "")
        else build_eddypro_coverage_audit(
            capability_matrix_path=args.capability_matrix or None,
            fixture_pack_path=args.fixture_pack or None,
            workspace_root=workspace_root,
            official_raw_evidence_pack=load_config_file(args.official_raw_evidence_pack)
            if args.official_raw_evidence_pack
            else None,
        )
    )
...
def _run_public_raw_importer_smoke_plan_cli(args: argparse.Namespace, parser: argparse.ArgumentParser) -> int:
    if not args.output:
        parser.error("--output is required with --build-public-raw-importer-smoke-plan.")
    workspace_root = Path(args.workspace_root) if args.workspace_root else None
    payload = build_public_raw_importer_smoke_plan(
        discovery_probe_path=args.public_ec_discovery_probe or None,
        manifest_path=args.public_ec_data_sources or None,
        workspace_root=workspace_root,
        max_sample_bytes=int(args.public_raw_smoke_max_sample_bytes or args.public_ec_sample_bytes or 65536),
    )
    output_path = Path(args.output)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
...
def _run_public_ec_acquisition_runbook_cli(args: argparse.Namespace, parser: argparse.ArgumentParser) -> int:
    if not args.output:
        parser.error("--output is required with --build-public-ec-acquisition-runbook.")
    workspace_root = Path(args.workspace_root) if args.workspace_root else None
    ...
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    return 0 if payload.get("artifact_type") == "public_ec_acquisition_runbook_v1" else 2
