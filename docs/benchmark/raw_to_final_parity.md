...
- Source-derived TOB1/SLT/native-binary conformance fixtures can be registered when public TOB1/SLT/native-binary bundles are unavailable. `eddypro_source_tob1_seconds_001` is generated from EddyPro engine TOB1 source anchors, `eddypro_source_slt_edisol_001` and `eddypro_source_slt_eddysoft_001` are generated from SLT EdiSol/EddySoft source anchors, and `eddypro_source_native_binary_mixed_001` is generated from generic binary source anchors. They preserve importer provenance, pass the raw-to-final harness, and are reported as `source_derived_conformance` rather than official field parity.
