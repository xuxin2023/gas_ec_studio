...
- GPS/PTP-style acquisition clock synchronization now applies offset/drift/event corrections before RP/FCC window slicing and daemon_telemetry v1 parses ptp4l/phc2sys/chrony/GPSD/PPS-style logs for lock, offset, jitter, hardware clock discipline offset/frequency, and dialect provenance; direct hardware clock discipline control and real target-host fixture validation remain incomplete.
- SmartFlux-like runtime hardening now has a headless runtime profile/watchdog manifest plus runtime_service v1 for queued headless batches, heartbeats, disk/queue telemetry, retry records, quarantined failures, delivery state, process telemetry, supervisor status, PTP/GPS/clock-discipline logs, hardware watchdog event logs, daemon_telemetry dialect provenance for systemd journal, Windows Event, chrony/phc2sys, GPSD/PPS, watchdogd, and provider JSONL, supervisor_integration v1 for systemd/Windows/manual status plus file/manual/Linux watchdog device/systemd notify/Windows recovery provider handoff, installable_runtime_profile v1 for auditable systemd/Windows Service dry-run deployment plans, runtime_deployment v1 for operator-gated install/rollback scripts, and runtime_deployment_feedback v1 for target-host post-install status/log evidence across report center, formal reports, delivery package, and exporter manifests; real hardware fixture validation remains incomplete.
