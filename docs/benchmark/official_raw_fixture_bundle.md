...
When a complete public TOB1/SLT bundle cannot be found, the registry now keeps moving with explicit source-derived conformance fixtures. `references/eddypro/source_derived/eddypro_source_tob1_seconds_001.tob1` is generated by `scripts/generate_source_derived_tob1_fixture.py` from EddyPro engine source anchors (`import_tob1.f90`, `m_fp2_to_float.f90`) and registered in `fixture_pack_v1.json` as `source_derived_conformance`. It validates TOB1 IEEE4 layout, leading ULONG preservation, and Campbell/LoggerNet `SECONDS`/`NANOSECONDS` timestamp decoding, but it is not a real field dataset and does not close official raw-to-final parity gates.
