...
class StudioController(QObject):
    ...
    def _rp_config_snapshot(self, *, precheck_only: bool) -> dict:
        ...
        config["footprint"] = {
            ...
            "z_m": float(footprint_step.get("z_m", 6.0) or 6.0),
            "canopy_height_m": float(footprint_step.get("canopy_height_m", 3.0) or 3.0),
    ...
    def _rp_method_summary(self, rp_result: RPRunResult | None) -> dict[str, object]:
        default = {
            ...
            "daemon_telemetry_status": "not_run",
            "supervisor_state": "",
            "ptp_lock_status": "",
            "gps_pps_lock_status": "",
            ...
            "runtime_deployment_feedback_status": "not_run",
            "runtime_deployment_feedback_state": "",
            "runtime_deployment_feedback_provenance": "",
            "runtime_deployment_feedback_summary": {},
        }
        ...
        daemon_telemetry_summary = dict(
            summary.get("daemon_telemetry_summary", {})
            or rp_result.artifacts.get("daemon_telemetry", {})
        ...
        installable_runtime_summary = dict(
            summary.get("installable_runtime_summary", {})
            or rp_result.artifacts.get("installable_runtime_profile", {})
            or supervisor_integration_summary.get("installable_runtime_profile", {})
            or {}
        )
        ...
        daemon_status = str(daemon_telemetry_summary.get("status") or default["daemon_telemetry_status"])
        ...
        runtime_deployment_feedback_summary = dict(
            summary.get("runtime_deployment_feedback_summary", {})
            or rp_result.artifacts.get("runtime_deployment_feedback", {})
            or supervisor_integration_summary.get("runtime_deployment_feedback", {})
            or {}
        )
        runtime_deployment_feedback_status = str(runtime_deployment_feedback_summary.get("status") or default["runtime_deployment_feedback_status"])
        runtime_deployment_feedback_state = str(dict(runtime_deployment_feedback_summary.get("service_status", {}) or {}).get("state", ""))
        runtime_deployment_feedback_provenance = str(runtime_deployment_feedback_summary.get("provenance", ""))
        if runtime_deployment_feedback_summary:
            runtime_deployment_feedback_provenance = (
                f"{runtime_deployment_feedback_provenance}; "
                f"service={runtime_deployment_feedback_state or '--'}; "
                f"target_apply={runtime_deployment_feedback_summary.get('target_apply_observed', '--')}"
            ).strip("; ")
        ...
        if installable_runtime_summary:
            installable_runtime_provenance = (
                ...
                f"deployment={runtime_deployment_status or '--'}; feedback={runtime_deployment_feedback_status or '--'}"
            ).strip("; ")
        ...
        return {
            ...
            "daemon_telemetry_status": daemon_status,
            ...
            "installable_runtime_profile_id": installable_runtime_profile_id,
            ...
            "runtime_deployment_feedback_status": runtime_deployment_feedback_status,
            "runtime_deployment_feedback_state": runtime_deployment_feedback_state,
            "runtime_deployment_feedback_provenance": runtime_deployment_feedback_provenance,
    ...
    def _build_report_payloads_from_run(self, run_result: SpectralRunResult) -> dict:
        ...
        installable_runtime_checks = list(installable_runtime_summary.get("checks", []) or [])
        runtime_deployment_feedback_summary = dict(rp_method_summary.get("runtime_deployment_feedback_summary", {}) or {})
        runtime_deployment_feedback_checks = list(runtime_deployment_feedback_summary.get("checks", []) or [])
        ...
        for check in daemon_telemetry_checks[:8]:
            payload = dict(check or {})
            method_compare_rows.append(
                (
        ...
        for check in installable_runtime_checks[:8]:
            payload = dict(check or {})
            method_compare_rows.append(
                (
                    f"install:{payload.get('check_id', '')}",
                    str(payload.get("status", "")),
                    f"measured={payload.get('measured', '--')}; threshold={payload.get('threshold', '--')}",
                )
            )
        for check in runtime_deployment_feedback_checks[:8]:
            payload = dict(check or {})
            method_compare_rows.append(
                (
                    f"deployment_feedback:{payload.get('check_id', '')}",
                    str(payload.get("status", "")),
                    f"measured={payload.get('measured', '--')}; threshold={payload.get('threshold', '--')}",
                )
            )
        ...
        reports["method_compare"] = {
            ...
            "metrics": [
                ...
                ("deployment_feedback", str(runtime_deployment_feedback_summary.get("status", "not_run"))),
    ...
    def _benchmark_cockpit_payload(self, run_result: SpectralRunResult) -> dict:
        ...
        if trace_gas_summary:
            ...
            table_rows.append(("trace_gas.ch4_normalization", trace_gas_summary.get("coefficient_profile_normalization_command", "--") or "--", "CH4 coefficient normalization command"))
            table_rows.append(("trace_gas.ch4_profile_provenance", trace_gas_summary.get("coefficient_profile_provenance", "--") or "--", "CH4 coefficient profile provenance"))
            ...
            table_rows.append(("trace_gas.n2o_normalization", trace_gas_summary.get("n2o_coefficient_profile_normalization_command", "--") or "--", "N2O coefficient normalization command"))
            table_rows.append(("trace_gas.n2o_profile_provenance", trace_gas_summary.get("n2o_coefficient_profile_provenance", "--") or "--", "N2O coefficient profile provenance"))
            table_rows.append(("trace_gas.n2o_spectral_factor", str(trace_gas_summary.get("n2o_spectral_correction_factor", "--")), "N2O spectral correction factor"))
            table_rows.append(("trace_gas.n2o_analyzer_factor", str(trace_gas_summary.get("n2o_analyzer_correction_factor", "--")), "N2O analyzer correction factor"))
            table_rows.append(("trace_gas.n2o_density_factor", str(trace_gas_summary.get("n2o_density_correction_factor", "--")), "N2O density correction factor"))
    ...
    def _benchmark_cockpit_payload(self, run_result: SpectralRunResult) -> dict:
        ...
        daemon_summary = dict(
            manifest_payload.get("daemon_telemetry_summary", {})
            or summary.get("daemon_telemetry_summary", {})
            or rp_result.artifacts.get("daemon_telemetry", {})
        ...
        installable_runtime_summary = dict(
            manifest_payload.get("installable_runtime_summary", {})
            or supervisor_summary.get("installable_runtime_profile", {})
            or rp_result.artifacts.get("installable_runtime_profile", {})
            or {}
        )
        ...
        runtime_deployment_feedback_summary = dict(
            manifest_payload.get("runtime_deployment_feedback_summary", {})
            or supervisor_summary.get("runtime_deployment_feedback", {})
            or rp_result.artifacts.get("runtime_deployment_feedback", {})
            or {}
        )
        ...
        if daemon_summary:
            table_rows.append(("daemon_telemetry.status", daemon_summary.get("status", "--"), "守护遥测状态"))
        ...
        if runtime_deployment_feedback_summary:
            feedback_service_state = dict(runtime_deployment_feedback_summary.get("service_status", {}) or {}).get("state", "--")
            table_rows.append(("runtime_deployment_feedback.status", runtime_deployment_feedback_summary.get("status", "--"), "target-host post-install feedback status"))
            table_rows.append(("runtime_deployment_feedback.service_state", feedback_service_state, "normalized systemd/Windows/manual service state"))
        for detail in per_window_detail:
        ...
        return {
            ...
            "steps": {
                ...
                "footprint": {
                    ...
                    "recommended": "默认推荐 kljun，z_m=6.0 m，canopy_height_m=3.0 m；复杂稳定度场景再切换到其他方法。",
                    "enabled": True,
                    "z_m": 6.0,
                    "canopy_height_m": 3.0,
                    "z0": 0.12,
